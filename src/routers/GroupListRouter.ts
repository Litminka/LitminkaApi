...
groupIdRouter.post(
    '/invite',
    new SendInviteRequest().send(),
    wrap(GroupInviteController.inviteUser)
);
groupIdRouter.delete(
    '/invite',
    new DeleteInviteRequest().send(),
    wrap(GroupInviteController.deleteInvite)
);

groupIdRouter.get(
    '/members',
    new GroupMemberRequest().send(),
    wrap(GroupMemberController.getMembers)
);
groupIdRouter.patch(
    '/members',
    new UpdateGroupMemberRequest().send(),
    wrap(GroupMemberController.updateState)
);
groupIdRouter.delete(
    '/members',
    new GroupMemberRequest().send(),
    wrap(GroupMemberController.leaveGroup)
);
groupIdRouter.delete(
    '/members/kick',
    new KickGroupMemberRequest().send(),
    wrap(GroupMemberController.kickUser)
);
...
groupListRouter.post(
    '/:animeId',
    new AddGroupAnimeListRequest().send(),
    wrap(GroupAnimeListController.add)
);
groupListRouter.patch(
    '/:animeId',
    new UpdateGroupAnimeListRequest().send(),
    wrap(GroupAnimeListController.update)
);
groupListRouter.delete(
    '/:animeId',
    new DeleteGroupAnimeListRequest().send(),
    wrap(GroupAnimeListController.delete)
);
...
invitesRouter.post(
    '/:inviteId/accept',
    new AcceptInviteRequest().send(),
    wrap(GroupInviteController.acceptInvite)
);
invitesRouter.delete(
    '/:inviteId/deny',
    new DenyInviteRequest().send(),
    wrap(GroupInviteController.denyInvite)
);
