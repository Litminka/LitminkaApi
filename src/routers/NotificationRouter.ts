...
router.get(
    '/user',
    new GetUserNotificationsRequest().send(),
    wrap(NotificationController.getUserNotifications)
);
router.get('', new GetNotificationsRequest().send(), wrap(NotificationController.getNotifications));
router.post(
    '/read',
    new ReadNotificationsRequest().send(),
    wrap(NotificationController.readNotifications)
);
